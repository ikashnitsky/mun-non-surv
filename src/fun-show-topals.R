...
show_topals = function(fit, emphasize_sd=FALSE, hue='red') {
 ...
 df_single  = data.frame(
  ...
  ggplot(data = df_single, aes(x=age,y=logmx_true)) +
      labs(x='Age',y='Log Death Rate',
           subtitle = paste(sum(fit$D),'deaths to',round(sum(fit$N)),'person-years')) +
      scale_x_continuous(breaks=c(0,1,seq(5,100,5)),minor_breaks = NULL) +
      scale_y_continuous(limits=c(-12,0),breaks=seq(-12,0,2),minor_breaks = NULL) +
      theme_bw()
 ...
 this_plot = this_plot + 
      geom_line(aes(x=age,y=std), lwd=0.5) +
