...
# load data
load("data/pooled_death_rates.rda")
load("data/pooled_std.rda")
...
save(df_fit, file = "data/topals-pooled-5y.rda")
...
load("data/death_rates.rda")
pop_size_19 <- death_rates %>% 
    filter(year == "2019") %>% 
    group_by(name) %>% 
    summarise(e = exposure %>% sum %>% divide_by(1e3) %>% round(2))

save(pop_size_19, file = "data/pop_size_19.rda")

# join population size
df_le_comapre <- df_le %>% 
    left_join(pop_size_19)

save(df_le_comapre, file = "data/df_le_comapre.rda")
...
load("data/pooled_1519_both.rda")
...
save(both_fit, file = "data/topals-both-1519.rda")

# calculate life expectancy
both_le <- both_fit %>% summarise(life_exp = logmx_fit %>% e0)

# compare to DST calculations
dst_le <- readxl::read_excel("data/dst-e0/HISBK-e0-mun.xlsx", skip = 3) %>% 
...
ggsave(filename = "figures/compare-life-exp-fit.pdf", gg_compare, 
       width = 7, height = 4)
...
ggsave("figures/maps-life-expectancy-check.pdf", maps_le, 
       width = 12, height = 6)
...
ggsave("figures/maps-le-q5065.pdf", maps_topals, 
       width = 12, height = 6)
...
ggsave("figures/maps-le-q5065-div.pdf", maps_topals_div, 
       width = 12, height = 6)
...
ggsave("figures/illustrate-topals-odense-kerteminde.pdf", illustrate_topals, 
       width = 5, height = 8)
...
ggsave("figures/illustrate-topals-copenhagen.pdf", gg_copenhagen, 
       width = 6, height = 5)
